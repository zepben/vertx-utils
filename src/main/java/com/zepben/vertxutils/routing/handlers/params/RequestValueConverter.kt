...
interface RequestValueConverter<T, R> {
    ...
    companion object {
        ...
        fun <T, R> create(expectedFormat: String, converter: Function<T, R>): RequestValueConverter<T, R> =
            object : RequestValueConverter<T, R> {

                override fun convert(param: T): R {
                    return converter.apply(param)
                }

                override fun expectedFormat(): String {
